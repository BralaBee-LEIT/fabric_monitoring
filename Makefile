...
generate-reports:
	...
	@if conda env list | grep -q "^$(ENV_NAME) "; then \
		conda run --no-capture-output -n $(ENV_NAME) python scripts/generate_reports_manual.py; \
	else \
		echo "$(RED)❌ Environment $(ENV_NAME) does not exist$(NC)"; \
		echo "$(YELLOW)Create it first with: make create$(NC)"; \
	fi
...
audit-sp-access:
	...
	@if conda env list | grep -q "^$(ENV_NAME) "; then \
		conda run --no-capture-output -n $(ENV_NAME) python scripts/audit_sp_access.py; \
	else \
		echo "$(RED)❌ Environment $(ENV_NAME) does not exist$(NC)"; \
		echo "$(YELLOW)Create it first with: make create$(NC)"; \
	fi
...
extract-details:
	...
	@if conda env list | grep -q "^$(ENV_NAME) "; then \
		...
		conda run --no-capture-output -n $(ENV_NAME) python scripts/extract_fabric_item_details.py $$WS_ARG; \
	else \
		echo "$(RED)❌ Environment $(ENV_NAME) does not exist$(NC)"; \
		echo "$(YELLOW)Create it first with: make create$(NC)"; \
	fi
