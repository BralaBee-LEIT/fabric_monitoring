...
def main():
    ...
    if ports_bad_state:
        ...
        print(
            f"Please stop the service running on ports {NEO4J_HTTP_PORT}/{NEO4J_BOLT_PORT} or manually edit docker-compose.yml."
        )
        sys.exit(1)
    ...
    try:
        subprocess.run(["docker", "compose", "up", "-d"], cwd=COMPOSE_DIR, env=local_env, check=True)
    ...
    for _ in range(30):
        try:
            ...
            result = subprocess.run(["curl", "-f", "-s", f"http://localhost:{NEO4J_HTTP_PORT}"], capture_output=True)
