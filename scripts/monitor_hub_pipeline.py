...
from usf_fabric_monitoring.core.pipeline import MonitorHubPipeline  # noqa: E402
...
Examples:
  ...
  python monitor_hub_pipeline.py --output exports/ # Custom output directory
    ...
    print(
        f"   • Monitoring Scope: {'Member workspaces only' if args.member_only else 'All tenant workspaces'}",
        flush=True,
    )
    print(
        f"   • API Strategy: {'Per-workspace loop' if args.member_only else 'Tenant-wide Power BI Admin API'}",
        flush=True,
    )
